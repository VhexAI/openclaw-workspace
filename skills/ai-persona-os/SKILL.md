...
name: ai-persona-os
version: 1.3.0
description: "The complete operating system for OpenClaw agents. Production-grade with enforced heartbeat protocol, traffic-light status indicators, context protection, security inoculation, team integration, proactive patterns, never-forget protocol, 8 operating rules, and 4 growth loops. One install. Complete system. Built by Jeff J Hunter."
...
## Heartbeat Protocol v2 (NEW in v1.3.0)

The #1 issue with v1.2.0: heartbeats fired but agents rubber-stamped `HEARTBEAT_OK` without running the protocol. v1.3.0 fixes this with an architecture that matches how OpenClaw actually works.

### What Changed

| v1.2.0 | v1.3.0 |
|--------|--------|
| 170-line HEARTBEAT.md (documentation) | ~20-line HEARTBEAT.md (imperative checklist) |
...
Every heartbeat that surfaces something uses this format:
```
🫀 HEARTBEAT — [Date, Time TZ]
...
→ [What was done or what needs attention]
```
...
2. (Optional) Add heartbeat prompt override — see `references/heartbeat-automation.md`
...
├── HEARTBEAT-template.md   → Tiny imperative checklist with 🟢🟡🔴 format (REWRITTEN v1.3.0)
